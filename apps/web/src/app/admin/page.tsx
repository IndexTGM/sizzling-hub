  ...
  useEffect(() => {
    if (open) {
      (async () => {
        ...
        const { data } = await sb.from("branches").select("id, name, slug").eq("is_active", true).order("name");
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        <span className="max-w-[180px] truncate">{selected?.name ?? "All Branches"}</span>
...
type AdminTab = "dashboard" | "orders" | "menu" | "profiles" | "banners" | "branches" | "images" | "reports";
...
const NAV_ITEMS: { tab: AdminTab; label: string; icon: React.JSX.Element }[] = [
  ...
  { tab: "branches", label: "Branches", icon: <path strokeLinecap="round" strokeLinejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /> },
    ...
    <div className="min-h-screen bg-[#f9fafb] flex">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${isMobile ? "ml-0" : sidebarOpen ? "ml-60" : "ml-16"}`}>
        ...
        <div className="flex-1 p-3 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
          {/* Branch selector + header for non-branch tabs */}
          {tab !== "branches" && (
            <div className="flex items-center justify-between mb-4">
              <h1 className="text-xl font-black text-gray-900 tracking-tight">{TAB_LABELS[tab]}</h1>
              <AdminBranchSelector
                selectedId={adminBranchId}
                onSelect={setAdminBranchId}
              />
            </div>
          )}
